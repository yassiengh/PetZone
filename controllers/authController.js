...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (req.body.verified === false) {
    return next(new AppError("Please activate your email ", 401));
  }
...
exports.verifyEmail = catchAsync(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: "Your Email token (valid for 10 min)",
      message,
    });
...
exports.emailVerfication = catchAsync(async (req, res, next) => {
  ...
  // 2) If token has not expired, and there is user, set the email as verified
  if (!user) {
    return next(new AppError("Token is invalid or has expired", 400));
  }
